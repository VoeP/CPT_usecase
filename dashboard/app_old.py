...
def render_mpl_image(plot_df, ids, use_predictions=False):
    ...
    for ax, sid in zip(axes, ids):
        ...
        # Probability twin axis (if exists)
        if 'confidence' in cpt.columns and cpt['confidence'].notna().any():
            ax_prob = ax.twiny()
            # Offset the spine
            ax_prob.spines["top"].set_position(("axes", 1.1))
            ax_prob.plot(cpt['confidence'], cpt['diepte'], color='green', lw=1.5, linestyle='--')
            ax_prob.set_xlabel('Confidence', color='green')
            ax_prob.spines['top'].set_color('green')
            ax_prob.tick_params(axis='x', colors='green')
            ax_prob.set_xlim(0, 1.05) # Prob is 0-1
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Checklist(
            id='prob-toggle',
            options=[{'label': 'Calculate & Plot Probabilities', 'value': 'prob'}],
            value=[],
            inputStyle={'margin-right': '6px'},
...
@app.callback(
    ...
    [State('sondering-dropdown', 'value'),
     ...
     State('prob-toggle', 'value')]
)
def run_prediction(n_clicks, selected_ids, raw_data_dict, prob_value):
    ...
    if model and le:
        ...
        if prob_value and 'prob' in prob_value:
            try:
                probs = model.predict_proba(X)
                # Take max probability as confidence
                processed_df['confidence'] = np.max(probs, axis=1)
            except Exception as e:
    ...
    if depth_col in cpt_subset.columns:
        ...
        cols_to_merge = ['sondering_id', 'depth_bin', 'predicted_label']
        if 'confidence' in processed_df.columns:
            cols_to_merge.append('confidence')

        merged_df = pd.merge(
            cpt_subset, 
            processed_df[cols_to_merge], 
